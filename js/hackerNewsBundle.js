...
/***/ function(module, exports, __webpack_require__) {
	...
	var HackerNewsFeed = React.createClass({
		displayName: 'HackerNewsFeed',

		getFeedIds: function getFeedIds() {
			$.ajax({
				method: 'GET',
				url: this.props.url,
		...
		loadFeedsFromServer: function loadFeedsFromServer(feedIdx) {
			//hnFeedsAr = [];
			$.ajax({
				method: 'GET',
		...
		render: function render() {
			...
			return React.createElement(
				...
				React.createElement(
					'button',
					{ className: 'btnHackerNews' },
					'Play'
				),
				'//TODO think issue with passing in state',
				React.createElement(HNFeedList, { data: this.state.data })
			);
		}
	});
	...
	var HNFeedList = React.createClass({
		displayName: 'HNFeedList',

		render: function render() {
			console.log('props.data: ' + this.props.data);
			console.log('props.data length: ' + this.props.data.length);
			console.log('type props.data: ' + _typeof(this.props.data));
			var hnFeedNodes = this.props.data.map(function (feed) {
				//var title = feed.title;
				//console.log('title: ' + title);
				return React.createElement(HNFeed, { key: feed.id, title: feed.title, description: feed.description, pubDate: feed.pubDate, link: feed.link });
			});

			return React.createElement(
				'div',
				{ className: 'hn-feed-list' },
				hnFeedNodes
			);
		}
	});

	ReactDOM.render(React.createElement(HackerNewsFeed, { url: 'https://hacker-news.firebaseio.com/v0/topstories.json?print=pretty' }), document.getElementById('hacker-news'));
